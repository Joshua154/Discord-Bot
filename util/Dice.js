...
module.exports = {
    ...
    getEmbed(interaction, ephemeral, times, user, type) {
        let embed = new EmbedBuilder().setAuthor({ name: `${user.username}`, iconURL: `${user.displayAvatarURL()}`});

        if(type === "dice"){
            let num = this.rollDice(100);
            embed.setTitle(num.toString());
            //embed.setColor(getColor(num).getHex)
            ...
            let roll = this.rollDice(100);
            let rollTwo = this.rollDice(100);
            ...
            for (const listKey in list) {
                if(rollTwo <= listKey && num === 0){
                    num = listKey;
                }
            }

            embed
                .setTitle(list[`${num}`].description + ` (_${rollTwo}_)`)
    ...
    getRow(displayCrit) {
        const crit = new ButtonBuilder()
            .setCustomId('diceButton_crit')
            .setLabel("Crit")
        ...
        let builder = new ActionRowBuilder()
            .addComponents(
                ...
                new ButtonBuilder()
                    .setCustomId('diceButton_bodyPart')
                    .setLabel("Body-Part")
                    .setStyle(ButtonStyle.Success),
            )

        if(displayCrit){
            builder.addComponents(crit)
        }

        return builder;
    },
    async sendEmbed(interaction, type) {
        ...
        let emb = this.getEmbed(interaction, interaction.options.getBoolean("private") ?? false, interaction.options.getInteger("times") ?? 1, user, type)

        interaction.reply({embeds: emb.embeds, ephemeral: emb.ephemeral, components: [this.getRow(((type === "dice") && isCrit(emb.embeds[0].data.title)))]})
...
/*function getColor(value){
    const first = new Color(46, 204, 113);
    const second = new Color(231, 76, 60);

    return new Color(Math.round((1-value) * first.r + value * second.r), Math.round((1-value) * first.g + value * second.g), Math.round((1-value) * first.b + value * second.b));
}*/
